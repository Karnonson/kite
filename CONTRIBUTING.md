...
Hi there! We're thrilled that you'd like to contribute to Kite. Contributions to this project are [released](https://help.github.com/articles/github-terms-of-service/#6-contributions-under-repository-license) to the public under the [project's open source license](LICENSE).
...
1. Test changes with the `specify` CLI commands (`/kite.specify`, `/kite.plan`, `/kite.tasks`) in your coding agent of choice
...
uv run kite init <temp-dir>/kite-test --integration <agent>
cd <temp-dir>/kite-test
...
4. **Run prerequisites first** — commands that depend on earlier commands (e.g., `/kite.tasks` requires `/kite.plan` which requires `/kite.specify`) must be run in order.
...
- src/kite_cli/*.py → CLI commands (`kite init`, `kite check`, `kite extension *`, `kite preset *`); test the affected CLI command and, for init/scaffolding changes, at minimum test /kite.specify
...
- presets/*/* → test preset scaffolding via `kite init` with the preset
- pyproject.toml → packaging/bundling; test `kite init` and verify bundled assets
...
- T1: /kite.command — (reason)
- T2: /kite.command — (reason)
~~~

## AI contributions in Kite

> [!IMPORTANT]
>
> If you are using **any kind of AI assistance** to contribute to Kite,
> it must be disclosed in the pull request or issue.

We welcome and encourage the use of AI tools to help improve Kite! Many valuable contributions have been enhanced with AI assistance for code generation, issue detection, and feature definition.

That being said, if you are using any kind of AI assistance (e.g., agents, ChatGPT) while contributing to Kite,
...
- **Clear rationale** - You can explain why the change is needed and how it fits within Kite's goals
...
- Generic suggestions that don't address specific Kite needs
