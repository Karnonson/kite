...
1. Run `{SCRIPT}` from repo root **once** (combined `--json --paths-only` mode / `-Json -PathsOnly`). Parse minimal JSON payload fields:
   ...
   - If JSON parsing fails, abort and instruct user to re-run `__KITE_COMMAND_SPECIFY__` or verify feature branch environment.
...
8. Report completion (after questioning loop ends or early termination):
   ...
   - If any Outstanding or Deferred remain, recommend whether to proceed to `__KITE_COMMAND_PLAN__` or run `__KITE_COMMAND_CLARIFY__` again later post-plan.
