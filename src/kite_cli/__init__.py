...
Usage:
    ...
    uvx kite-cli.py init .
    uvx kite-cli.py init --here

Or install globally:
    uv tool install --from kite-cli.py kite-cli
...
def _version_callback(value: bool):
    if value:
        console.print(f"kite {get_kite_version()}")
        raise typer.Exit()
...
def callback(
    ...
    if ctx.invoked_subcommand is None and "--help" not in sys.argv and "-h" not in sys.argv:
        show_banner()
        console.print(Align.center("[dim]Run 'kite --help' for usage information[/dim]"))
        console.print()
...
def _locate_core_pack() -> Path | None:
    ...
    templates/, scripts/ etc. into kite_cli/core_pack/ at build time.
...
def _install_shared_infra(
    ...
    Copies ``.kite/scripts/`` and ``.kite/templates/`` from the
    bundled core_pack or source checkout.  Tracks all installed files
    in ``kite.manifest.json``.

    Page templates are processed to resolve ``__KITE_COMMAND_<NAME>__``
    ...
    core = _locate_core_pack()
    manifest = IntegrationManifest("kite", project_path, version=get_kite_version())
    ...
    if scripts_src.is_dir():
        dest_scripts = project_path / ".kite" / "scripts"
    ...
    if templates_src.is_dir():
        dest_templates = project_path / ".kite" / "templates"
...
def ensure_executable_scripts(project_path: Path, tracker: StepTracker | None = None) -> None:
    """Ensure POSIX .sh scripts under .kite/scripts and .kite/extensions (recursively) have execute bits (no-op on Windows)."""
    if os.name == "nt":
        return  # Windows: skip silently
    scan_roots = [
        project_path / ".kite" / "scripts",
        project_path / ".kite" / "extensions",
    ]
    failures: list[str] = []
    updated = 0
...
def ensure_constitution_from_template(project_path: Path, tracker: StepTracker | None = None) -> None:
    ...
    memory_constitution = project_path / ".kite" / "memory" / "constitution.md"
    template_constitution = project_path / ".kite" / "templates" / "constitution-template.md"
...
INSTALL_PROFILES = frozenset({"minimal", "standard", "full"})

_GENERATED_PROJECT_ROOT_ENTRIES = frozenset({
    ".git",
    ".gitignore",
    ".kite",
    "AGENTS.md",
...
def save_init_options(project_path: Path, options: dict[str, Any]) -> None:
    """Persist the CLI options used during ``kite init``.

    Writes a small JSON file to ``.kite/init-options.json`` so that
...
def _normalize_install_profile(profile: str) -> str:
    """Normalize and validate an install profile name."""
    normalized = profile.strip().lower()
    if normalized in INSTALL_PROFILES:
        return normalized
    console.print(
        f"[red]Error:[/red] Unknown install profile '{profile}'. "
        "Valid values: minimal, standard, full."
    )
    raise typer.Exit(1)


def _require_kite_project(project_root: Path) -> None:
    """Ensure the current directory is a Kite project."""
    if not (project_root / ".kite").exists():
        console.print("[red]Error:[/red] Not a Kite project (no .kite/ directory)")
        raise typer.Exit(1)


def _project_root_entries(project_root: Path) -> list[str]:
    """Return current top-level entry names for lightweight project detection."""
    return sorted(entry.name for entry in project_root.iterdir())


def _load_package_json(project_path: Path) -> dict[str, Any]:
    """Load package.json when present and valid."""
...
def _detect_package_manager(
    ...
    """Return the dominant package manager for the repository when recognizable."""
    indicators = [
        ("pnpm-lock.yaml", "pnpm"),
        ("yarn.lock", "yarn"),
        ("bun.lockb", "bun"),
    ...
    for relative_path, package_manager in indicators:
        if (project_path / relative_path).exists():
            return package_manager

    package_json = package_json if package_json is not None else _load_package_json(project_path)
    package_manager_field = package_json.get("packageManager")
    if isinstance(package_manager_field, str):
        package_manager = package_manager_field.split("@", 1)[0].strip().lower()
...
def _detect_frameworks(package_json: dict[str, Any]) -> list[str]:
    """Detect a small set of common frontend frameworks from package.json."""
    package_names: set[str] = set()
    for section in ("dependencies", "devDependencies"):
        value = package_json.get(section)
        if isinstance(value, dict):
            package_names.update(name for name in value if isinstance(name, str))
    ...
    return sorted({label for package_name, label in framework_map.items() if package_name in package_names})
...
def _build_validation_commands(
    ...
    """Infer a small validation command set from well-known package.json scripts."""
    scripts = package_json.get("scripts")
    if not isinstance(scripts, dict) or package_manager not in {"npm", "pnpm", "yarn", "bun"}:
        return []
    ...
    command_prefix = script_commands[package_manager]
    known_scripts = [
        ("lint", "Lint"),
        ("test", "Tests"),
        ("build", "Build"),
    ...
    for script_name, label in known_scripts:
        script_value = scripts.get(script_name)
        if not isinstance(script_value, str) or not script_value.strip():
            continue
        validation_commands.append(
            {
                "id": f"package-{script_name}",
                "label": label,
                "command": [*command_prefix, script_name],
                "source": "package.json",
            }
        )
...
def _build_project_context(
    project_path: Path,
    preexisting_root_entries: list[str],
) -> dict[str, Any]:
    """Build a lightweight repository summary for validation and prompting."""
    package_json = _load_package_json(project_path)
    package_manager = _detect_package_manager(project_path, package_json)
    frameworks = _detect_frameworks(package_json)
    validation_commands = _build_validation_commands(package_manager, package_json)

    brownfield = any(
        entry not in _GENERATED_PROJECT_ROOT_ENTRIES
        for entry in preexisting_root_entries
    )
    if not brownfield:
        brownfield = bool(package_manager or frameworks or validation_commands)
...
def _write_project_context(project_path: Path, context: dict[str, Any]) -> None:
    """Write `.kite/project-context.json` without following symlink escapes."""
    destination = project_path / PROJECT_CONTEXT_FILE
    ...
    try:
        ...
        console.print(
            f"[red]Error:[/red] Refusing to write {PROJECT_CONTEXT_FILE} outside the project."
        )
        raise typer.Exit(1)

    if destination.exists() and destination.is_symlink():
        console.print(
            f"[red]Error:[/red] Refusing to overwrite symlinked {PROJECT_CONTEXT_FILE}."
        )
        raise typer.Exit(1)
...
def write_project_context(project_path: Path, preexisting_root_entries: list[str]) -> None:
    """Persist a lightweight repository summary for later validation and prompting."""
    _write_project_context(
        project_path,
        _build_project_context(project_path, preexisting_root_entries),
    )


def _read_project_context(project_root: Path) -> dict[str, Any] | None:
    """Read `.kite/project-context.json` when present."""
    context_path = project_root / PROJECT_CONTEXT_FILE
    ...
    try:
        ...
        console.print(f"[red]Error:[/red] Could not read {PROJECT_CONTEXT_FILE}: {exc}")
        raise typer.Exit(1)

    if not isinstance(data, dict):
        console.print(
            f"[red]Error:[/red] {PROJECT_CONTEXT_FILE} must contain a JSON object."
        )
        raise typer.Exit(1)
    return data
...
def _merge_project_context(
    ...
    """Merge refreshed detection into existing context without dropping custom data."""
    if existing is None:
        return detected

    merged = dict(existing)
    merged["schema_version"] = detected["schema_version"]
    merged["summary"] = detected["summary"]

    if not isinstance(merged.get("brownfield"), bool):
        merged["brownfield"] = detected["brownfield"]
    if not isinstance(merged.get("validation_commands"), list):
        merged["validation_commands"] = detected["validation_commands"]
    return merged
...
def _load_project_context(
    ...
    *,
    ...
    """Load the persisted project context, optionally refreshing it first."""
    existing = _read_project_context(project_root)
    if refresh_context and (project_root / ".kite").exists():
        detected = _build_project_context(
            project_root,
            _project_root_entries(project_root),
        )
        refreshed = _merge_project_context(existing, detected)
        _write_project_context(project_root, refreshed)
        return refreshed
...
def _run_project_validation(
    ...
    *,
    ...
    """Run project validation commands from `.kite/project-context.json` when present."""
    context = _load_project_context(project_root, refresh_context=refresh_context)
    if not context:
        return False
    ...
    for item in commands:
        ...
        label = item.get("label") or item.get("id") or "Validation"
        command = item.get("command")
        if not (isinstance(command, list) and all(isinstance(part, str) for part in command)):
            console.print(f"[red]✗[/red] {label}")
            console.print("Invalid validation command entry in project context; use an array of strings.")
            failures += 1
            continue
        ...
        try:
            ...
            failures += 1
            console.print(f"[red]✗[/red] {label}")
            console.print(f"Could not run validation command: {exc}")
            continue
    ...
    if failures:
        console.print(f"\n[red]{failures} validation command(s) failed[/red]")
        raise typer.Exit(1)
...
def _current_install_profile(project_root: Path) -> str:
    """Return the current install profile for a Kite project."""
...
def _set_install_profile(project_root: Path, profile: str) -> str:
    """Persist a validated install profile into init-options.json."""
    normalized = _normalize_install_profile(profile)
    options = load_init_options(project_root)
    options["profile"] = normalized
...
def load_init_options(project_path: Path) -> dict[str, Any]:
    """Load the init options previously saved by ``kite init``.
...
def init(
    ...
    profile: str = typer.Option("standard", "--profile", help="Install profile: minimal, standard, or full"),
    ...
    Examples:
        ...
        kite init --here --integration claude    # Alternative syntax for current directory
        kite init --here --integration codex --integration-options="--skills"
        kite init --here --integration codebuddy
        kite init --here --integration vibe      # Initialize with Mistral Vibe support
        kite init --here
        kite init --here --force  # Skip confirmation when current directory not empty
        kite init my-project --integration claude   # Claude installs skills by default
        kite init --here --integration gemini
        kite init my-project --integration generic --integration-options="--commands-dir .myagent/commands/"  # Bring your own agent; requires --commands-dir
        kite init my-project --integration claude --preset healthcare-compliance  # With preset
    """
    ...
    if ai_assistant and ai_assistant.startswith("--"):
        ...
        console.print("[yellow]Example:[/yellow] kite init --integration claude --here")
    ...
    if ai_commands_dir and ai_commands_dir.startswith("--"):
        ...
        console.print("[yellow]Example:[/yellow] kite init --integration generic --integration-options=\"--commands-dir .myagent/commands/\"")
        raise typer.Exit(1)
    ...
    if no_git:
        console.print(
            ...
            "— use the [bold]kite extension[/bold] commands to install or enable the git extension if needed.[/yellow]"
        )
    ...
    if ai_skills and not ai_assistant:
        ...
        console.print("[yellow]Usage:[/yellow] kite init <project> --ai <agent> --ai-skills")
        raise typer.Exit(1)
    ...
    if selected_ai == "generic" and not integration_options:
        if not ai_commands_dir:
            ...
            console.print('[dim]Example: kite init my-project --integration generic --integration-options="--commands-dir .myagent/commands/"[/dim]')
            raise typer.Exit(1)
    ...
    with Live(tracker.render(), console=console, refresh_per_second=8, transient=True) as live:
        ...
        try:
            ...
            manifest = IntegrationManifest(
                resolved_integration.key, project_path, version=get_kite_version()
            )
            ...
            integration_parsed_options["profile"] = profile
            ...
            # Write .kite/integration.json
            integration_json = project_path / ".kite" / "integration.json"
            integration_json.parent.mkdir(parents=True, exist_ok=True)
            integration_json.write_text(json.dumps({
                "integration": resolved_integration.key,
                "version": get_kite_version(),
            }, indent=2) + "\n", encoding="utf-8")
            ...
            # Install bundled kite workflow
            try:
                bundled_wf = _locate_bundled_workflow("kite")
                if bundled_wf:
                    from .workflows.catalog import WorkflowRegistry
                    from .workflows.engine import WorkflowDefinition
                    wf_registry = WorkflowRegistry(project_path)
                    if wf_registry.is_installed("kite"):
                        tracker.complete("workflow", "already installed")
                    else:
                        import shutil as _shutil
                        dest_wf = project_path / ".kite" / "workflows" / "kite"
                        ...
                        wf_registry.add("kite", {
                            "name": definition.name,
                            "version": definition.version,
                            "description": definition.description,
                            "source": "bundled",
                        })
                        tracker.complete("workflow", "kite installed")
                else:
            ...
            if preset:
                try:
                    ...
                    kite_ver = get_kite_version()

                    # Try local directory first, then bundled, then catalog
                    local_path = Path(preset).resolve()
                    if local_path.is_dir() and (local_path / "preset.yml").exists():
                        preset_manager.install_from_directory(local_path, kite_ver)
                    else:
                        bundled_path = _locate_bundled_preset(preset)
                        if bundled_path:
                            preset_manager.install_from_directory(bundled_path, kite_ver)
                        else:
                            ...
                            if not pack_info:
                                ...
                                try:
                                    ...
                                    preset_manager.install_from_zip(zip_path, kite_ver)
            ...
            write_project_context(project_path, [item.name for item in existing_items])
...
def check(
    run_validation: bool = typer.Option(
        False,
        "--run-validation",
        help="Run validation commands from .kite/project-context.json.",
    ),
    tools: bool = typer.Option(
        False,
        "--tools",
        help="Run the environment/tool check (default behavior).",
    ),
    refresh_context: bool = typer.Option(
        True,
        "--refresh-context/--no-refresh-context",
        help="Refresh .kite/project-context.json before project validation.",
    ),
):
    """Check project validation commands or required tools."""
    project_root = Path.cwd()
    if run_validation and tools:
        console.print("[red]Error:[/red] Choose either --run-validation or --tools, not both.")
        raise typer.Exit(1)

    if run_validation:
        _require_kite_project(project_root)
        if _run_project_validation(project_root, refresh_context=refresh_context):
            return
        console.print("[yellow]No project validation commands found.[/yellow]")
        return
    ...
    console.print("\n[bold green]Kite CLI is ready to use![/bold green]")
...
profile_app = typer.Typer(
    name="profile",
    help="Show or update the project's install profile.",
    add_completion=False,
    invoke_without_command=True,
)
...
def profile_callback(ctx: typer.Context) -> None:
    """Show the current profile when no subcommand is provided."""
...
def profile_show() -> None:
    """Display the current install profile for this project."""
    project_root = Path.cwd()
    _require_kite_project(project_root)
    profile = _current_install_profile(project_root)
    console.print(f"Profile: [cyan]{profile}[/cyan]")


@profile_app.command("set")
def profile_set(
    profile: str = typer.Argument(..., help="Install profile to persist: minimal, standard, or full"),
    upgrade: bool = typer.Option(
        False,
        "--upgrade",
        help="Immediately run integration upgrade with the saved profile",
    ),
) -> None:
    """Persist a new install profile for this project."""
    project_root = Path.cwd()
    _require_kite_project(project_root)

    current = _current_install_profile(project_root)
    normalized = _normalize_install_profile(profile)
    if normalized == current:
        console.print(f"Profile is already set to [cyan]{normalized}[/cyan].")
    else:
        _set_install_profile(project_root, normalized)
        console.print(f"Profile updated to [cyan]{normalized}[/cyan].")
...
def version():
    ...
    panel = Panel(
        info_table,
        title="[bold cyan]Kite CLI Information[/bold cyan]",
        border_style="cyan",
        padding=(1, 2)
    )
...
def _get_installed_version() -> str:
    """Return the installed kite-cli distribution version or 'unknown'.
    ...
    intentional for `kite self check`, which should reason about the
    ...
    try:
        return importlib.metadata.version("kite-cli")
...
def _detect_self_install_method(executable: str) -> str:
    """Best-effort detection of how kite-cli was installed."""
    normalized = executable.replace("\\", "/").lower()
    if "/uv/tools/" in normalized and "/kite-cli/" in normalized:
        return "uv tool"
    if "/pipx/venvs/" in normalized and "/kite-cli/" in normalized:
        return "pipx"
    return "unknown"


def _self_upgrade_command(install_method: str, tag: str) -> list[str] | None:
    """Return the installer command for the detected install method."""
    repo_ref = f"git+https://github.com/Karnonson/kite.git@{tag}"
    if install_method == "uv tool":
        return ["uv", "tool", "install", "kite-cli", "--force", "--from", repo_ref]
    if install_method == "pipx":
        return ["pipx", "install", "--force", repo_ref]
    return None
...
def self_upgrade(
    dry_run: bool = typer.Option(False, "--dry-run", help="Preview the upgrade command without running it."),
    apply: bool = typer.Option(False, "--apply", help="Run the detected installer command."),
    yes: bool = typer.Option(False, "--yes", "-y", help="Skip the confirmation prompt when used with --apply."),
) -> None:
    """Plan or run a kite-cli self-upgrade based on the install method."""
    installed = _get_installed_version()
    tag, failure_reason = _fetch_latest_release_tag()
    ...
    latest_normalized = _normalize_tag(tag)
    install_method = _detect_self_install_method(sys.executable)
    command = _self_upgrade_command(install_method, tag)
    ...
    if installed != "unknown" and not _is_newer(latest_normalized, installed):
        console.print(f"[green]Up to date:[/green] {installed}")
        return
    ...
    if command is None:
        console.print("[yellow]Install method could not safely detect an automatic upgrade path.[/yellow]")
        console.print("Try one of these manual commands:")
        console.file.write(
            "  uv tool install kite-cli --force --from "
            f"git+https://github.com/Karnonson/kite.git@{tag}\n"
        )
        console.file.write(
            "  pipx install --force "
            f"git+https://github.com/Karnonson/kite.git@{tag}\n"
        ...
        console.file.flush()
        raise typer.Exit(1 if should_apply else 0)

    command_str = shlex.join(command)
    console.print("Upgrade command:")
    console.file.write(f"  {command_str}\n")
    console.file.flush()

    if not should_apply:
        console.print("Dry run only. Re-run with --apply to execute this command.")
        return

    if not yes:
        confirmed = typer.confirm("Run this command now?", default=False, abort=False)
        if not confirmed:
    ...
    console.print("[green]Self upgrade command completed.[/green]")
...
INTEGRATION_JSON = ".kite/integration.json"


def _read_integration_json(project_root: Path) -> dict[str, Any]:
    """Load ``.kite/integration.json``.  Returns ``{}`` when missing."""
...
def _write_integration_json(
    ...
    """Write ``.kite/integration.json`` for *integration_key*."""
...
def _remove_integration_json(project_root: Path) -> None:
    """Remove ``.kite/integration.json`` if it exists."""
    path = project_root / INTEGRATION_JSON
    if path.exists():
        path.unlink()
...
def _resolve_script_type(project_root: Path, script_type: str | None) -> str:
    ...
    if isinstance(saved, str) and saved.strip():
        return _normalize_script_type(saved, ".kite/init-options.json")
    return "ps" if os.name == "nt" else "sh"


def _resolve_integration_profile(project_root: Path, profile: str | None) -> str:
    """Resolve the integration install profile from CLI input or project options."""
    if profile is not None:
        return _normalize_install_profile(profile)
    return _current_install_profile(project_root)


def _integration_options_with_profile(
    integration: Any,
    raw_options: str | None,
    profile: str,
) -> dict[str, Any]:
    """Parse integration options and inject the resolved install profile."""
    parsed_options = _parse_integration_options(integration, raw_options) if raw_options else None
    options = dict(parsed_options or {})
...
def integration_list(
    ...
    specify_dir = project_root / ".kite"
    if not specify_dir.exists():
        console.print("[red]Error:[/red] Not a spec-kit project (no .kite/ directory)")
    ...
    if installed_key:
        ...
        console.print("Install one with: [cyan]kite integration install <key>[/cyan]")
...
def integration_install(
    ...
    profile: str | None = typer.Option(None, "--profile", help="Install profile: minimal, standard, or full"),
    ...
    if installed_key and installed_key == key:
        ...
        console.print("Run [cyan]kite integration uninstall[/cyan] first, then reinstall.")
        raise typer.Exit(0)

    if installed_key:
        console.print(f"[red]Error:[/red] Integration '{installed_key}' is already installed.")
        console.print(f"Run [cyan]kite integration uninstall[/cyan] first, or use [cyan]kite integration switch {key}[/cyan].")
        raise typer.Exit(1)

    selected_script = _resolve_script_type(project_root, script)
    effective_profile = _resolve_integration_profile(project_root, profile)
    ...
    # is installed.
    parsed_options = _integration_options_with_profile(
        integration,
        integration_options,
        effective_profile,
    )
    ...
    manifest = IntegrationManifest(
        integration.key, project_root, version=get_kite_version()
    )
...
def integration_uninstall(
    ...
    manifest_path = project_root / ".kite" / "integrations" / f"{key}.manifest.json"
    ...
    try:
        ...
        console.print(
            ...
            f"[cyan]kite integration uninstall {key}[/cyan] to clear stale metadata, "
            f"then run [cyan]kite integration install {key}[/cyan] to regenerate."
        )
        console.print(f"[dim]Details:[/dim] {exc}")
        raise typer.Exit(1)
...
def integration_switch(
    ...
    if installed_key:
        ...
        manifest_path = project_root / ".kite" / "integrations" / f"{installed_key}.manifest.json"
        ...
        if current_integration and manifest_path.exists():
            ...
            try:
                ...
                console.print(
                    ...
                    f"run [cyan]kite integration uninstall {installed_key}[/cyan], then retry."
                )
                raise typer.Exit(1)
            ...
            console.print(
                f"Run [cyan]kite integration uninstall {installed_key}[/cyan] to clear metadata, "
                f"then retry [cyan]kite integration switch {target}[/cyan]."
            )
            raise typer.Exit(1)
    ...
    manifest = IntegrationManifest(
        target_integration.key, project_root, version=get_kite_version()
    )
...
def integration_upgrade(
    ...
    if installed_key and installed_key != key:
        ...
        console.print(f"Use [cyan]kite integration switch {key}[/cyan] instead.")
        raise typer.Exit(1)
    ...
    if not manifest_path.exists():
        ...
        console.print(f"Run [cyan]kite integration install {key}[/cyan] to perform a fresh install.")
        raise typer.Exit(0)
...
def preset_list():
    ...
    if not installed:
        ...
        console.print("  [cyan]kite preset add <pack-name>[/cyan]")
        return
...
def preset_add(
    ...
    try:
        if dev:
            ...
            manifest = manager.install_from_directory(dev_path, kite_version, priority)
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                manifest = manager.install_from_zip(zip_path, kite_version, priority)
            ...
            if bundled_path:
                ...
                manifest = manager.install_from_directory(bundled_path, kite_version, priority)
                console.print(f"[green]✓[/green] Preset '{manifest.name}' v{manifest.version} installed (priority {priority})")
            else:
...
def preset_info(
    ...
    console.print(f"  Install with: [cyan]kite preset add {preset_id}[/cyan]")
    console.print()
...
def preset_disable(
    ...
    console.print(f"To re-enable: kite preset enable {preset_id}")
...
def preset_catalog_list():
    ...
    config_path = project_root / ".kite" / "preset-catalogs.yml"
    user_config_path = Path.home() / ".kite" / "preset-catalogs.yml"
    if os.environ.get("KITE_PRESET_CATALOG_URL"):
        console.print("[dim]Catalog configured via KITE_PRESET_CATALOG_URL environment variable.[/dim]")
    else:
        try:
        ...
        if proj_loaded:
            ...
            try:
                ...
                user_loaded = False
            if user_loaded:
                console.print("[dim]Config: ~/.kite/preset-catalogs.yml[/dim]")
            else:
                console.print("[dim]Using built-in default catalog stack.[/dim]")
                console.print(
                    "[dim]Add .kite/preset-catalogs.yml to customize.[/dim]"
                )
...
def preset_catalog_add(
    ...
    """Add a catalog to .kite/preset-catalogs.yml."""
    from .presets import PresetCatalog, PresetValidationError
    ...
    for existing in catalogs:
        if isinstance(existing, dict) and existing.get("name") == name:
            ...
            console.print("Use 'kite preset catalog remove' first, or choose a different name.")
            raise typer.Exit(1)
...
def preset_catalog_remove(
    ...
    """Remove a catalog from .kite/preset-catalogs.yml."""
    project_root = Path.cwd()
...
def _resolve_installed_extension(
    ...
    if len(name_matches) == 1:
        ...
        console.print(f"  [bold]kite extension {command_name} <extension-id>[/bold]")
        raise typer.Exit(1)
    else:
...
def extension_list(
    ...
    if not installed and not (available or all_extensions):
        ...
        console.print("  kite extension add <extension-name>")
        return
    ...
    if available or all_extensions:
        ...
        console.print("  [cyan]kite extension add <name>[/cyan]")
...
def catalog_list():
    ...
    config_path = project_root / ".kite" / "extension-catalogs.yml"
    user_config_path = Path.home() / ".kite" / "extension-catalogs.yml"
    if os.environ.get("KITE_CATALOG_URL"):
        console.print("[dim]Catalog configured via KITE_CATALOG_URL environment variable.[/dim]")
    else:
        try:
            proj_loaded = config_path.exists() and catalog._load_catalog_config(config_path) is not None
        ...
        if proj_loaded:
            ...
            try:
                ...
                user_loaded = False
            if user_loaded:
                console.print("[dim]Config: ~/.kite/extension-catalogs.yml[/dim]")
            else:
                console.print("[dim]Using built-in default catalog stack.[/dim]")
                console.print(
                    "[dim]Add .kite/extension-catalogs.yml to customize.[/dim]"
                )
...
def catalog_add(
    ...
    """Add a catalog to .kite/extension-catalogs.yml."""
    from .extensions import ExtensionCatalog, ValidationError
    ...
    for existing in catalogs:
        if isinstance(existing, dict) and existing.get("name") == name:
            console.print(f"[yellow]Warning:[/yellow] A catalog named '{name}' already exists.")
            console.print("Use 'kite extension catalog remove' first, or choose a different name.")
            raise typer.Exit(1)
...
def catalog_remove(
    ...
    """Remove a catalog from .kite/extension-catalogs.yml."""
    project_root = Path.cwd()
    ...
    try:
        with console.status(f"[cyan]Installing extension: {extension}[/cyan]"):
            if dev:
                ...
                manifest = manager.install_from_directory(source_path, kite_version, priority=priority)
                ...
                download_dir = project_root / ".kite" / "extensions" / ".cache" / "downloads"
                ...
                try:
                    ...
                    manifest = manager.install_from_zip(zip_path, kite_version, priority=priority)
                except urllib.error.URLError as e:
                ...
                if bundled_path is not None:
                    manifest = manager.install_from_directory(bundled_path, kite_version, priority=priority)
                else:
                    ...
                    if not ext_info:
                        ...
                        console.print("  kite extension search")
                        raise typer.Exit(1)
                    ...
                    if bundled_path is None:
                        ...
                        if not ext_info.get("_install_allowed", True):
                            ...
                            console.print(
                                ...
                                f"(install_allowed: true) in .kite/extension-catalogs.yml."
                            )
                            raise typer.Exit(1)
        ...
        console.print(f"   Check: .kite/extensions/{manifest.id}/")
...
def extension_remove(
    ...
    if not force:
        ...
        console.print(f"   • Extension directory: .kite/extensions/{extension_id}/")
        if not keep_config:
    ...
    if success:
        ...
        if keep_config:
            console.print(f"\nConfig files preserved in .kite/extensions/{extension_id}/")
        else:
            console.print(f"\nConfig files backed up to .kite/extensions/.backup/{extension_id}/")
        console.print(f"\nTo reinstall: kite extension add {extension_id}")
    else:
    ...
    try:
        ...
        if not results:
            ...
            if query or tag or author or verified:
                ...
                console.print("  • kite extension search (show all)")
            raise typer.Exit(0)
        ...
        for ext in results:
            ...
            if install_allowed:
                console.print(f"\n  [cyan]Install:[/cyan] kite extension add {ext['id']}")
            else:
                console.print(f"\n  [yellow]⚠[/yellow]  Not directly installable from '{catalog_name}'.")
                console.print(
                    f"  Add to an approved catalog with install_allowed: true, "
                    f"or install from a ZIP URL: kite extension add {ext['id']} --from <zip-url>"
                )
            console.print()
...
def extension_info(
    ...
    if resolved_installed_id:
        ...
        console.print(f"[dim]Priority:[/dim] {priority}")
        console.print(f"\nTo remove: kite extension remove {resolved_installed_id}")
        return
...
def _print_extension_info(ext_info: dict, manager):
    ...
    if ext_info.get('requires'):
        ...
        if reqs.get('kite_version'):
            console.print(f"  • Kite: {reqs['kite_version']}")
        if reqs.get('tools'):
    ...
    if is_installed:
        ...
        console.print(f"[dim]Priority:[/dim] {priority}")
        console.print(f"\nTo remove: kite extension remove {ext_info['id']}")
    elif install_allowed:
        console.print("[yellow]Not installed[/yellow]")
        console.print(f"\n[cyan]Install:[/cyan] kite extension add {ext_info['id']}")
    else:
        catalog_name = ext_info.get("_catalog_name", "community")
        console.print("[yellow]Not installed[/yellow]")
        console.print(
            f"\n[yellow]⚠[/yellow]  '{ext_info['id']}' is available in the '{catalog_name}' catalog "
            f"but not in your approved catalog. Add it to .kite/extension-catalogs.yml "
        ...
        for update in updates_available:
            ...
            try:
                ...
                try:
                    ...
                    _ = manager.install_from_zip(zip_path, kite_version)
...
def extension_disable(
    ...
    console.print(f"To re-enable: kite extension enable {extension_id}")
...
def workflow_run(
    ...
    if state.status.value == "paused":
        console.print(f"\nResume with: [cyan]kite workflow resume {state.run_id}[/cyan]")
...
def workflow_list():
    ...
    if not installed:
        ...
        console.print("  [cyan]kite workflow add <workflow-id>[/cyan]")
        return
...
def workflow_add(
    ...
    registry = WorkflowRegistry(project_root)
    workflows_dir = project_root / ".kite" / "workflows"
...
def workflow_remove(
    ...
    workflow_dir = project_root / ".kite" / "workflows" / workflow_id
    if workflow_dir.exists():
        import shutil
