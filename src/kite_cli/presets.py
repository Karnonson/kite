"""
Preset Manager for Kite

Handles installation, removal, and management of Kite presets.
...
def _substitute_core_template(
    ...
    Args:
        ...
        cmd_name: Full command name (e.g. "kite.git.feature" or "kite.specify").
    ...
    # Derive the short name (strip "kite." prefix) used by core command templates.
    short_name = cmd_name
    if short_name.startswith("kite."):
        short_name = short_name[len("kite."):]
    ...
    #    command name (e.g. kite.selftest.extension → commands/selftest.md).
...
class PresetManifest:
    ...
    def _validate(self):
        ...
        if "kite_version" not in requires:
            raise PresetValidationError("Missing requires.kite_version")
        ...
        for tmpl in provides["templates"]:
            ...
            if tmpl["type"] == "command":
                # Commands use dot notation (e.g. kite.specify)
    ...
    @property
    def requires_kite_version(self) -> str:
        """Get required spec-kit version range."""
        return self.data["requires"]["kite_version"]
...
class PresetRegistry:
    ...
    def __init__(self, packs_dir: Path):
        ...
        Args:
            packs_dir: Path to .kite/presets/ directory
        """
...
class PresetManager:
    ...
    def __init__(self, project_root: Path):
        ...
        self.presets_dir = project_root / ".kite" / "presets"
    ...
    def check_compatibility(
        ...
        Args:
            ...
            kite_version: Current spec-kit version
        ...
        """
        required = manifest.requires_kite_version
        current = pkg_version.Version(kite_version)
    ...
    def _register_commands(
        ...
        # Command names follow the pattern: kite.<ext-id>.<cmd-name>
        # Core commands (e.g. kite.specify) have only one dot — always register.
        extensions_dir = self.project_root / ".kite" / "extensions"
        filtered = []
        for cmd in command_templates:
            parts = cmd["name"].split(".")
            if len(parts) >= 3 and parts[0] == "kite":
                ext_id = parts[1]
            ...
            if not has_composition:
                ...
                if not registered:
                    ...
                    if source.startswith("extension:"):
                        ...
                        ext_dir = self.project_root / ".kite" / "extensions" / ext_id
    ...
    def _register_command_from_path(
        ...
        if source_id and not source_id.startswith("preset:"):
            try:
                from .extensions import ExtensionManifest
                for ext_dir in (self.project_root / ".kite" / "extensions").iterdir():
    ...
    def _get_skills_dir(self) -> Optional[Path]:
        ...
        Reads ``.kite/init-options.json`` to determine whether skills
    ...
    def _skill_names_for_command(cmd_name: str) -> tuple[str, str]:
        ...
        if raw_short_name.startswith("kite."):
            raw_short_name = raw_short_name[len("kite."):]

        modern_skill_name = f"kite-{raw_short_name.replace('.', '-')}"
        legacy_skill_name = f"kite.{raw_short_name}"
    ...
    def _skill_title_from_command(cmd_name: str) -> str:
        ...
        title_name = cmd_name
        if title_name.startswith("kite."):
            title_name = title_name[len("kite."):]
    ...
    def _register_skills(
        ...
        for cmd_tmpl in filtered:
            ...
            # Derive the short command name (e.g. "specify" from "kite.specify")
            raw_short_name = cmd_name
            if raw_short_name.startswith("kite."):
                raw_short_name = raw_short_name[len("kite."):]
    ...
    def _unregister_skills(self, skill_names: List[str], preset_dir: Path) -> None:
        ...
        core_templates_dir = self.project_root / ".kite" / "templates" / "commands"
        init_opts = load_init_options(self.project_root)
        if not isinstance(init_opts, dict):
            init_opts = {}
        selected_ai = init_opts.get("ai")
        ...
        for skill_name in skill_names:
            # Derive command name from skill name (kite-specify -> specify)
            short_name = skill_name
            if short_name.startswith("kite-"):
                short_name = short_name[len("kite-"):]
            elif short_name.startswith("kite."):
                short_name = short_name[len("kite."):]
    ...
    def install_from_directory(
        ...
        self.check_compatibility(manifest, kite_version)

        if self.registry.is_installed(manifest.id):
            raise PresetError(
                f"Preset '{manifest.id}' is already installed. "
                f"Use 'kite preset remove {manifest.id}' first."
            )
    ...
    def install_from_zip(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            return self.install_from_directory(pack_dir, kite_version, priority)
        ...
        if removed_cmd_names:
            try:
                ...
                warnings.warn(
                    ...
                    f"or run 'kite preset add' to refresh.",
                    stacklevel=2,
                )
...
class PresetCatalog:
    ...
    DEFAULT_CATALOG_URL = "https://raw.githubusercontent.com/Karnonson/kite/main/presets/catalog.json"
    COMMUNITY_CATALOG_URL = "https://raw.githubusercontent.com/Karnonson/kite/main/presets/catalog.community.json"
    ...
    def _make_request(self, url: str):
        ...
        Delegates to :func:`kite_cli._github_http.build_github_request`.
        """
        from kite_cli._github_http import build_github_request
        return build_github_request(url)

    def _open_url(self, url: str, timeout: int = 10):
        """Open a URL with GitHub auth, stripping the header on cross-host redirects.

        Delegates to :func:`kite_cli._github_http.open_github_url`.
        """
        from kite_cli._github_http import open_github_url
    ...
    def get_active_catalogs(self) -> List[PresetCatalogEntry]:
        ...
        Resolution order:
        1. KITE_PRESET_CATALOG_URL env var — single catalog replacing all defaults
        2. Project-level .kite/preset-catalogs.yml
        3. User-level ~/.kite/preset-catalogs.yml
        ...
        # 1. KITE_PRESET_CATALOG_URL env var replaces all defaults
        if env_value := os.environ.get("KITE_PRESET_CATALOG_URL"):
            ...
            return [PresetCatalogEntry(url=catalog_url, name="custom", priority=1, install_allowed=True, description="Custom catalog via KITE_PRESET_CATALOG_URL")]

        # 2. Project-level config overrides all defaults
        project_config_path = self.project_root / ".kite" / "preset-catalogs.yml"
    ...
    def download_pack(
        ...
        if pack_info.get("bundled") and not pack_info.get("download_url"):
            ...
            raise PresetError(
                ...
                f"Use 'kite preset add {pack_id}' to install from the bundled package, "
...
class PresetResolver:
    ...
    Resolution order:
    1. .kite/templates/overrides/          - Project-local overrides
    2. .kite/presets/<preset-id>/          - Installed presets
    3. .kite/extensions/<ext-id>/templates/ - Extension-provided templates
    4. .kite/templates/                    - Core templates (shipped with Kite)
    """
    ...
    def __init__(self, project_root: Path):
        ...
        """
        self.project_root = project_root
        self.templates_dir = project_root / ".kite" / "templates"
        self.presets_dir = project_root / ".kite" / "presets"
        self.overrides_dir = self.templates_dir / "overrides"
        self.extensions_dir = project_root / ".kite" / "extensions"
    ...
    def _core_stem(template_name: str) -> Optional[str]:
        ...
        Commands use dot notation (e.g. ``kite.specify``), but core
        command files are named by stem (e.g. ``specify.md``).  Returns
        the stem if *template_name* follows the ``kite.<stem>`` pattern,
        or ``None`` otherwise.
        """
        if template_name.startswith("kite."):
            return template_name[len("kite."):]
        return None
    ...
    def resolve(
        ...
        # kite's built-in command/template files and must always be checked
        # so that strategy:wrap presets can locate {CORE_TEMPLATE}.
        from kite_cli import _locate_core_pack  # local import to avoid cycles
    ...
    def resolve_extension_command_via_manifest(self, cmd_name: str) -> Optional[Path]:
        ...
        (e.g. ``kite.selftest.extension`` → ``commands/selftest.md``).
    ...
    def _find_bundled_core(
        ...
        ``.kite/templates/`` doesn't contain the core file.
        """
        try:
            from kite_cli import _locate_core_pack
        except ImportError:
            return None
