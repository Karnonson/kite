"""Command step — dispatches a Kite command to an integration CLI."""
...
from kite_cli.workflows.base import StepBase, StepContext, StepResult, StepStatus
from kite_cli.workflows.expressions import evaluate_expression


class CommandStep(StepBase):
    """Default step type — invokes a Kite command via the integration CLI.
    ...
    execute the command by name (e.g. ``/kite.specify`` or
    ``/kite-specify``) rather than reading the file contents.
        ...
        if dispatch_result is not None:
            ...
            return StepResult(
                ...
                error=(
                    ...
                    f"Install the CLI tool or check 'kite integration list'."
                ),
            )
    ...
    def _try_dispatch(
        ...
        slash-command invocation (e.g. ``/kite.specify`` for
        markdown agents, ``/kite-specify`` for skills agents),
