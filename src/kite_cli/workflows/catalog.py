...
class WorkflowRegistry:
    ...
    def __init__(self, project_root: Path) -> None:
        ...
        self.workflows_dir = project_root / ".kite" / "workflows"
...
class WorkflowCatalog:
    ...
    1. ``KITE_WORKFLOW_CATALOG_URL`` env var (overrides all)
    2. Project-level ``.kite/workflow-catalogs.yml``
    3. User-level ``~/.kite/workflow-catalogs.yml``
    4. Built-in defaults (official + community)
    """

    DEFAULT_CATALOG_URL = (
        "https://raw.githubusercontent.com/Karnonson/kite/main/"
    ...
    def get_active_catalogs(self) -> list[WorkflowCatalogEntry]:
        ...
        env_url = os.environ.get("KITE_WORKFLOW_CATALOG_URL", "").strip()
        if env_url:
            ...
            return [
                WorkflowCatalogEntry(
                    ...
                    description="From KITE_WORKFLOW_CATALOG_URL",
                )
            ]

        # 2. Project-level config
        project_config = self.project_root / ".kite" / "workflow-catalogs.yml"
        ...
        # 3. User-level config
        home = Path.home()
        user_config = home / ".kite" / "workflow-catalogs.yml"
    ...
    def add_catalog(self, url: str, name: str | None = None) -> None:
        ...
        self._validate_catalog_url(url)
        config_path = self.project_root / ".kite" / "workflow-catalogs.yml"
