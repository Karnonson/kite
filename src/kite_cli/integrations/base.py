...
- ``SkillsIntegration`` — concrete base for integrations that install
  commands as agent skills (``kite-<name>/SKILL.md`` layout).
"""
...
class IntegrationBase(ABC):
    ...
    * ``context_file``     — path (relative to project root) of the agent
    ...
    def build_command_invocation(self, command_name: str, args: str = "") -> str:
        """Build the native slash-command invocation for a Kite command.

        The CLI tools discover and execute commands from installed files
        on disk.  This method builds the invocation string the CLI
        expects — e.g. ``"/kite.specify my-feature"`` for markdown
        agents or ``"/kite-specify my-feature"`` for skills agents.

        *command_name* may be a full dotted name like
        ``"kite.specify"``, an extension command like
        ``"kite.git.commit"``, or a bare stem like ``"specify"``.
        """
        stem = command_name
        if stem.startswith("kite."):
            stem = stem[len("kite."):]

        invocation = f"/kite.{stem}"
        if args:
    ...
    def dispatch_command(
        ...
        *,
        ...
        """Dispatch a Kite command through this integration's CLI.
    ...
    def command_filename(self, template_name: str) -> str:
        ...
        *template_name* is the stem of the source file (e.g. ``"plan"``).
        Default: ``kite.{template_name}.md``.  Subclasses override
        to change the extension or naming convention.
        """
        return f"kite.{template_name}.md"
    ...
    def install_scripts(
        ...
        ``.kite/integrations/<key>/scripts/`` in the project.  Shell
        ...
        *manifest*.
        ...
        created: list[Path] = []
        scripts_dest = project_root / ".kite" / "integrations" / self.key / "scripts"
    ...
    def upsert_context_section(
        ...
        ``<!-- KITE START -->`` and ``<!-- KITE END -->`` markers
    ...
    def remove_context_section(self, project_root: Path) -> bool:
        ...
        # For .mdc files, treat Kite-generated frontmatter-only content as empty
        if ctx_path.suffix == ".mdc":
            import re
    ...
    def resolve_command_refs(content: str, separator: str = ".") -> str:
        """Replace ``__KITE_COMMAND_<NAME>__`` placeholders with invocations.

        Each placeholder encodes a command name in upper-case with
        underscores (e.g. ``__KITE_COMMAND_PLAN__``,
        ``__KITE_COMMAND_GIT_COMMIT__``).  The replacement uses
        *separator* to join the segments:

        * ``separator="."`` → ``/kite.plan``, ``/kite.git.commit``
        * ``separator="-"`` → ``/kite-plan``, ``/kite-git-commit``
        """
        return re.sub(
            r"__KITE_COMMAND_([A-Z][A-Z0-9_]*)__",
            lambda m: "/kite" + separator + m.group(1).lower().replace("_", separator),
            content,
        )
    ...
    def process_template(
        ...
        7. Rewrite paths: ``scripts/`` → ``.kite/scripts/`` etc.
        8. Replace ``__KITE_COMMAND_<NAME>__`` with invocation strings
        """
...
class TomlIntegration(IntegrationBase):
    ...
    def command_filename(self, template_name: str) -> str:
        ...
        return f"kite.{template_name}.toml"
...
class YamlIntegration(IntegrationBase):
    ...
    def command_filename(self, template_name: str) -> str:
        ...
        return f"kite.{template_name}.yaml"
    ...
    def _human_title(identifier: str) -> str:
        ...
        Strips a leading ``kite.`` prefix and replaces ``.``, ``-``,
        and ``_`` with spaces before title-casing.
        """
        text = identifier
        if text.startswith("kite."):
            text = text[len("kite.") :]
...
class SkillsIntegration(IntegrationBase):
    ...
    Skills use the ``kite-<name>/SKILL.md`` directory layout following
    ...
    ``kite-<name>/SKILL.md`` file with skills-oriented frontmatter.
    """
    ...
    def build_command_invocation(self, command_name: str, args: str = "") -> str:
        """Skills use ``/kite-<stem>`` (hyphenated directory name)."""
        stem = command_name
        if stem.startswith("kite."):
            stem = stem[len("kite."):]
    ...
    def setup(
        ...
        **opts: Any,
        ...
        Creates ``kite-<name>/SKILL.md`` for each shared command
        ...
        for src_file in templates:
            ...
            skill_name = f"kite-{command_name.replace('.', '-')}"
            ...
            skill_content = (
                ...
                f"compatibility: {_quote('Requires spec-kit project structure with .kite/ directory')}\n"
                f"metadata:\n"
