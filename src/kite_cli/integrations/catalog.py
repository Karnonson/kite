...
class IntegrationCatalog:
    ...
    DEFAULT_CATALOG_URL = (
        "https://raw.githubusercontent.com/Karnonson/kite/main/integrations/catalog.json"
    )
    COMMUNITY_CATALOG_URL = (
        "https://raw.githubusercontent.com/Karnonson/kite/main/integrations/catalog.community.json"
    )
    CACHE_DURATION = 3600  # 1 hour

    def __init__(self, project_root: Path) -> None:
        self.project_root = project_root
        self.cache_dir = project_root / ".kite" / "integrations" / ".cache"
    ...
    def get_active_catalogs(self) -> List[IntegrationCatalogEntry]:
        ...
        Resolution:
        1. ``KITE_INTEGRATION_CATALOG_URL`` env var
        2. Project ``.kite/integration-catalogs.yml``
        3. User ``~/.kite/integration-catalogs.yml``
        4. Built-in defaults (built-in + community)
        """
        import sys

        env_value = os.environ.get("KITE_INTEGRATION_CATALOG_URL", "").strip()
        if env_value:
            ...
            return [
                IntegrationCatalogEntry(
                    ...
                    description="Custom catalog via KITE_INTEGRATION_CATALOG_URL",
                )
            ]

        project_cfg = self.project_root / ".kite" / "integration-catalogs.yml"
        catalogs = self._load_catalog_config(project_cfg)
        if catalogs is not None:
            return catalogs

        user_cfg = Path.home() / ".kite" / "integration-catalogs.yml"
...
class IntegrationDescriptor:
    ...
    def _validate(self) -> None:
        ...
        if "kite_version" not in requires:
            raise IntegrationDescriptorError(
                "Missing requires.kite_version"
            )
        if not isinstance(requires["kite_version"], str) or not requires["kite_version"].strip():
            raise IntegrationDescriptorError(
                "requires.kite_version must be a non-empty string"
            )
