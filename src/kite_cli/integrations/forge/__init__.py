...
- Uses a hyphenated frontmatter `name` value (e.g., `kite-foo-bar`) for shell compatibility, especially with ZSH
"""
...
def format_forge_command_name(cmd_name: str) -> str:
    ...
    Examples:
        ...
        'kite-plan'
        >>> format_forge_command_name("kite.plan")
        'kite-plan'
        >>> format_forge_command_name("kite-plan")
        'kite-plan'
        >>> format_forge_command_name("kite.my-extension.example")
        'kite-my-extension-example'
        >>> format_forge_command_name("kite-my-extension-example")
        'kite-my-extension-example'
        >>> format_forge_command_name("kite.jira.sync-status")
        'kite-jira-sync-status'
    
    Args:
        cmd_name: Command name in dot notation (kite.foo.bar), 
                  hyphenated format (kite-foo-bar), or plain name (foo)
    
    Returns:
        Hyphenated command name with 'kite-' prefix
    """
    # Already in hyphenated format - return as-is (idempotent)
    if cmd_name.startswith("kite-"):
        return cmd_name
    
    # Strip 'kite.' prefix if present
    short_name = cmd_name
    ...
    # Return with 'kite-' prefix
    return f"kite-{short_name}"
