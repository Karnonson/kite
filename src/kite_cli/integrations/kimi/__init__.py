...
Kimi uses the ``.kimi/skills/kite-<name>/SKILL.md`` layout with
``/skill:kite-<name>`` invocation syntax.

Includes legacy migration logic for projects initialised before Kimi
moved from dotted skill directories (``kite.xxx``) to hyphenated
(``kite-xxx``).
"""
...
class KimiIntegration(SkillsIntegration):
    ...
    def options(cls) -> list[IntegrationOption]:
        return [
            ...
            IntegrationOption(
                ...
                is_flag=True,
                default=False,
                help="Migrate legacy dotted skill dirs (kite.xxx → kite-xxx)",
            ),
        ]
    ...
    def setup(
        ...
        **opts: Any,
        ...
        # Run base setup first so hyphenated targets (kite-*) exist,
...
def _migrate_legacy_kimi_dotted_skills(skills_dir: Path) -> tuple[int, int]:
    """Migrate legacy Kimi dotted skill dirs (kite.xxx) to hyphenated format.
    ...
    for legacy_dir in sorted(skills_dir.glob("kite.*")):
        if not legacy_dir.is_dir():
            continue
        if not (legacy_dir / "SKILL.md").exists():
            continue

        suffix = legacy_dir.name[len("kite."):]
        if not suffix:
            continue

        target_dir = skills_dir / f"kite-{suffix.replace('.', '-')}"
