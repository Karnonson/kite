...
EXTENSION_COMMAND_NAME_PATTERN = re.compile(r"^kite\.([a-z0-9-]+)\.([a-z0-9-]+)$")

REINSTALL_COMMAND = "uv tool install kite-cli --force --from git+https://github.com/Karnonson/kite.git"
...
class ExtensionManifest:
    ...
    def _validate(self):
        ...
        if "kite_version" not in requires:
            raise ValidationError("Missing requires.kite_version")
        ...
        for cmd in commands:
            ...
            if not EXTENSION_COMMAND_NAME_PATTERN.match(cmd["name"]):
                ...
                if corrected:
                    self.warnings.append(
                        ...
                        f"'kite.{{extension}}.{{command}}'. Registering as '{corrected}'. "
                    ...
                    raise ValidationError(
                        ...
                        "must follow pattern 'kite.{extension}.{command}'"
                    )
            ...
            # (e.g. 'kite.verify' short aliases used by existing extensions).
        ...
        # an alias-form ref (ext.cmd → kite.ext.cmd).  Always emit a warning when
        ...
        for hook_name, hook_data in self.data.get("hooks", {}).items():
            ...
            # Step 2: lift alias-form '{ext_id}.cmd' to canonical 'kite.{ext_id}.cmd'.
            parts = after_rename.split(".")
            if len(parts) == 2 and parts[0] == ext["id"]:
                final_ref = f"kite.{ext['id']}.{parts[1]}"
            else:
    ...
    def _try_correct_command_name(name: str, ext_id: str) -> Optional[str]:
        ...
        Handles the two legacy formats used by community extensions:
          - 'kite.command'  → 'kite.{ext_id}.command'
          - '{ext_id}.command' → 'kite.{ext_id}.command'
        ...
        parts = name.split('.')
        if len(parts) == 2:
            if parts[0] == 'kite' or parts[0] == ext_id:
                candidate = f"kite.{ext_id}.{parts[1]}"
...
class ExtensionRegistry:
    ...
    def __init__(self, extensions_dir: Path):
        ...
        Args:
            extensions_dir: Path to .kite/extensions/ directory
        """
...
class ExtensionManager:
    ...
    def _collect_manifest_command_names(manifest: ExtensionManifest) -> Dict[str, str]:
        ...
        - primary commands must use the canonical `kite.{extension}.{command}` shape
    ...
    def _register_extension_skills(
        ...
        for cmd_info in manifest.commands:
            ...
            if short_name_raw.startswith("kite."):
                short_name_raw = short_name_raw[len("kite."):]
            skill_name = f"kite-{short_name_raw.replace('.', '-')}"
    ...
    def install_from_directory(
        ...
        if self.registry.is_installed(manifest.id):
            raise ExtensionError(
                ...
                f"Use 'kite extension remove {manifest.id}' first."
            )
    ...
    def install_from_zip(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            return self.install_from_directory(extension_dir, kite_version, priority=priority)
...
class CommandRegistrar:
    ...
    def register_commands_for_agent(
        ...
        context_note = f"\n<!-- Extension: {manifest.id} -->\n<!-- Config: .kite/extensions/{manifest.id}/ -->\n"
...
class ExtensionCatalog:
    ...
    DEFAULT_CATALOG_URL = "https://raw.githubusercontent.com/Karnonson/kite/main/extensions/catalog.json"
    COMMUNITY_CATALOG_URL = "https://raw.githubusercontent.com/Karnonson/kite/main/extensions/catalog.community.json"
    ...
    def get_active_catalogs(self) -> List[CatalogEntry]:
        ...
        Resolution order:
        1. KITE_CATALOG_URL env var — single catalog replacing all defaults
        2. Project-level .kite/extension-catalogs.yml
        3. User-level ~/.kite/extension-catalogs.yml
        ...
        # 1. KITE_CATALOG_URL env var replaces all defaults for backward compat
        if env_value := os.environ.get("KITE_CATALOG_URL"):
            ...
            return [CatalogEntry(url=catalog_url, name="custom", priority=1, install_allowed=True, description="Custom catalog via KITE_CATALOG_URL")]

        # 2. Project-level config overrides all defaults
        project_config_path = self.project_root / ".kite" / "extension-catalogs.yml"
...
class ConfigManager:
    ...
    2. Project config (.kite/extensions/{ext-id}/{ext-id}-config.yml)
    3. Local config (.kite/extensions/{ext-id}/local-config.yml) - gitignored
    4. Environment variables (KITE_{EXT_ID}_{KEY})
    """
    ...
    def __init__(self, project_root: Path, extension_id: str):
        ...
        self.extension_dir = project_root / ".kite" / "extensions" / extension_id
    ...
    def _get_env_config(self) -> Dict[str, Any]:
        ...
        KITE_{EXT_ID}_{SECTION}_{KEY}

        For example:
        - KITE_JIRA_CONNECTION_URL
...
class HookExecutor:
    ...
    def __init__(self, project_root: Path):
        ...
        self.project_root = project_root
        self.extensions_dir = project_root / ".kite" / "extensions"
        self.config_file = project_root / ".kite" / "extensions.yml"
    ...
    def _load_init_options(self) -> Dict[str, Any]:
        ...
        Uses the shared helper from kite_cli and caches values per executor
    ...
    def _skill_name_from_command(command: Any) -> str:
        """Map a command id like kite.plan to kite-plan skill name."""
        if not isinstance(command, str):
            return ""
        command_id = command.strip()
        if not command_id.startswith("kite."):
            return ""
