"""
Agent Command Registrar for Kite
...
class CommandRegistrar:
    ...
    def _adjust_script_paths(self, frontmatter: dict) -> dict:
        ...
        ``memory/``) to the ``.kite/...`` paths used in generated projects.
    ...
    def rewrite_project_relative_paths(text: str) -> str:
        ...
        for old, new in (
            ...
            ("../../templates/", ".kite/templates/"),
        ):
            text = text.replace(old, new)

        # Only rewrite top-level style references so extension-local paths like
        # ".kite/extensions/<ext>/scripts/..." remain intact.
        text = re.sub(r'(^|[\s`"\'(])(?:\.?/)?memory/', r"\1.kite/memory/", text)
        text = re.sub(r'(^|[\s`"\'(])(?:\.?/)?scripts/', r"\1.kite/scripts/", text)
        text = re.sub(
            r'(^|[\s`"\'(])(?:\.?/)?templates/', r"\1.kite/templates/", text
        )

        return text.replace(".kite/.kite/", ".kite/").replace(
            ".kite.specify/", ".kite/"
        )
    ...
    def render_yaml_command(
        ...
        """
        from kite_cli.integrations.base import YamlIntegration
    ...
    def write_copilot_prompt(project_root: Path, cmd_name: str) -> None:
        ...
        Args:
            project_root: Path to project root
            cmd_name: Command name (e.g. 'kite.my-ext.example')
        """
        ...
        for agent_name, cmd_names in registered_commands.items():
            ...
            for cmd_name in cmd_names:
                ...
                if cmd_file.exists():
                    ...
                    # (e.g. .agents/skills/kite-ext-cmd/SKILL.md). Remove the
