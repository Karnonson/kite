...
Presets are stackable, priority-ordered collections of template and command overrides for Kite. They let you customize both the artifacts produced by the Spec-Driven Development workflow (specs, plans, tasks, checklists, constitutions) and the commands that guide the LLM in creating them — without forking or modifying core files.

## How It Works

When Kite needs a template (e.g. `spec-template`), it walks a resolution stack:

1. `.kite/templates/overrides/` — project-local one-off overrides
2. `.kite/presets/<preset-id>/templates/` — installed presets (sorted by priority)
...
Template resolution happens **at runtime** — although preset files are copied into `.kite/presets/<id>/` during installation, Kite walks the resolution stack on every template lookup rather than merging templates into a single location.
...
kite preset add healthcare-compliance

# Install from a local directory (for development)
kite preset add --dev ./my-preset

# Install with a specific priority (lower = higher precedence)
kite preset add healthcare-compliance --priority 5
...
kite preset info healthcare-compliance

# Remove a preset
kite preset remove healthcare-compliance
```
...
```bash
kite preset add enterprise-safe --priority 10      # base layer
kite preset add healthcare-compliance --priority 5  # overrides enterprise-safe
kite preset add pm-workflow --priority 1            # overrides everything
```
...
Presets are discovered through catalogs. By default, Kite uses the official and community catalogs:
...
kite preset catalog add https://example.com/catalog.json --name my-org --install-allowed

# Remove a catalog
kite preset catalog remove my-org
```
...
4. Test locally with `kite preset add --dev .`
5. Verify with `kite preset resolve spec-template`

## Environment Variables

| Variable | Description | Default |
|----------|-------------|---------|
| `KITE_PRESET_CATALOG_URL` | Override the full catalog stack with a single URL (replaces all defaults) | Built-in default stack |
...
# Search a private catalog added via `kite preset catalog add`
kite preset search my-template

# Install from a private catalog
kite preset add my-template
```
...
|------|-------|-------------|
| `.kite/preset-catalogs.yml` | Project | Custom catalog stack for this project |
| `~/.kite/preset-catalogs.yml` | User | Custom catalog stack for all projects |
...
- **Conflict detection** — `kite preset lint` / `kite preset doctor` for detecting composition conflicts.
