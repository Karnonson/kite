...
A custom preset for Kite. Copy this directory and customize it to create your own.
...
| `kite.specify` | command | Custom specification command (overrides core) |
| `kite.myext.myextcmd` | command | Override of the myext extension's myextcmd command |
...
4. Test locally: `kite preset add --dev ./my-preset`
5. Verify resolution: `kite preset resolve spec-template`
6. Remove when done testing: `kite preset remove my-preset`
...
- `requires.kite_version` — version constraint (e.g. `>=0.1.0`)
...
- **command** — AI agent workflow prompts (e.g. kite.specify, kite.plan)
