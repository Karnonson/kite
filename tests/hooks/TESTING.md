...
This directory contains a mock project to verify that LLM agents correctly identify and execute hook commands defined in `.kite/extensions.yml`.
...
2. Ask it to generate tasks for the current directory:
   > "Please follow `/kite.tasks` for the `./tests/hooks` directory."
3. **Expected Behavior**: 
   - Before doing any generation, the LLM should notice the `AUTOMATIC Pre-Hook` in `.kite/extensions.yml` under `before_tasks`.
...
1. In the same (or new) chat, ask the LLM to implement the tasks:
   > "Please follow `/kite.implement` for the `./tests/hooks` directory."
2. **Expected Behavior**:
