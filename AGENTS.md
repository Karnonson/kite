...
| Skill directories (`kite-<name>/SKILL.md`) | `SkillsIntegration` |
...
In `src/kite_cli/integrations/__init__.py`, add one import and one `_register()` call inside `_register_builtins()`. Both lists are alphabetical:
...
Create two thin wrapper scripts in `src/kite_cli/integrations/<package_dir>/scripts/` that delegate to the shared context-update scripts. Each is ~25 lines of boilerplate.
...
_root="$_script_dir"
while [ "$_root" != "/" ] && [ ! -d "$_root/.kite" ]; do _root="$(dirname "$_root")"; done
if [ -z "${REPO_ROOT:-}" ]; then
  if [ -d "$_root/.kite" ]; then
    REPO_ROOT="$_root"
  else
    git_root="$(git rev-parse --show-toplevel 2>/dev/null || true)"
    if [ -n "$git_root" ] && [ -d "$git_root/.kite" ]; then
      REPO_ROOT="$git_root"
    else
      REPO_ROOT="$_root"
    fi
...
exec "$REPO_ROOT/.kite/scripts/bash/update-agent-context.sh" <key>
```
...
if (-not $repoRoot -or -not (Test-Path (Join-Path $repoRoot '.kite'))) {
    $repoRoot = $scriptDir
    $fsRoot = [System.IO.Path]::GetPathRoot($repoRoot)
    while ($repoRoot -and $repoRoot -ne $fsRoot -and -not (Test-Path (Join-Path $repoRoot '.kite'))) {
        $repoRoot = Split-Path -Parent $repoRoot
    }
}

& "$repoRoot/.kite/scripts/powershell/update-agent-context.ps1" -AgentType <key>
```
...
kite init my-project --integration <key>
...
# Uninstall cleanly
cd my-project && kite integration uninstall <key>
```
...
|---|---|---|
| `command_filename(template_name)` | Custom file naming or extension | Copilot → `kite.{name}.agent.md` |
| `options()` | Integration-specific CLI flags via `--integration-options` | Codex → `--skills` flag, Copilot → `--skills` flag |
| `setup()` | Custom install logic (companion files, settings merge) | Copilot → `.agent.md` + `.prompt.md` + `.vscode/settings.json` (default) or `kite-<name>/SKILL.md` (skills mode) |
| `teardown()` | Custom uninstall logic | Rarely needed; base handles manifest-tracked files |

**Example — Copilot (fully custom `setup`):**

Copilot extends `IntegrationBase` directly because it creates `.agent.md` commands, companion `.prompt.md` files, and merges `.vscode/settings.json`. It also supports a `--skills` mode that scaffolds `kite-<name>/SKILL.md` under `.github/skills/` using composition with an internal `_CopilotSkillsHelper`. See `src/kite_cli/integrations/copilot/__init__.py` for the full implementation.
...
- `post_process_skill_content()` injects a `mode: kite.<stem>` frontmatter field
- `build_command_invocation()` returns `/kite-<stem>` instead of bare args
...
# Skills mode: kite-<name>/SKILL.md under .github/skills/
